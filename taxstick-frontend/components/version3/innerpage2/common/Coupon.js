...
const Coupon = ({ serviceData, bgImage = '/v3/About3/goalbg.svg', bgColor = '' }) => {
    ...
    useEffect(() => {
        ...
        setRefresh(null)
    }, [serviceData?.coupon_code?.end_duration, refresh,endDate, serviceData?.coupon_code?.start_duration, serviceData?.coupon_code?.name,status, serviceData?.coupon_code?._id,todayDate, startDate]);
