        ...
        <div className="position-relative hover:outline-none focus:outline-none hover:border-blue-600">
            <input
                ...
                accept={"image/png, image/svg, image/gif, image/webp, image/jpeg" + ( pdf ? ' application/pdf' : '')} onChange={handleChange}
                   aria-label="Upload"/>
            {url && (
                <Image role="button" src={url} alt="" onClick={() => setImage(url)} className="position-absolute" id="inputGroupFileAddon04" style={{height: 33, width: 48, right: 1, top: 1, zIndex: 10}}/>
            )}
            <Modal width={800} visible={image} onCancel={() => setImage(undefined)} footer={null}
                ...
                <Image className="w-100" style={{minHeight: 400}} src={image} alt={''}/>
            </Modal>
        </div>
    )
