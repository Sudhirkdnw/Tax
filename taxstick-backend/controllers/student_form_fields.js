...
exports.createStudentFormFiled = async (req, res, next) => {
    ...
    if (role === 'admin' || role === 'accountant') {
        ...
        try {
            let { input_name, input_type, placeholder, field_required, status, step_name, userRoleID, action_for, select_options, link } = req.body;

            const field_name = input_name;
            input_name = input_name.trim().split(' ').join('_');

            const insertData = { field_name, input_name, input_type, placeholder, field_required, status, step_name, select_options, link }
            
            if (action_for === 'all') {

                const newStudentFormField = await StudentFormField.create([insertData], { session });

                if (!newStudentFormField) return res.status(400).json({ message: 'Wrong input! try again..', status: false });
...
exports.getOneStudentFormField = async (req, res, next) => {
    ...
    if (role === 'admin' || role === 'accountant') {
        try {
            ...
            if (!studentFormField) return res.status(404).json({ message: 'Filed Not found', status: false });
...
exports.getAllStudentFormField = async (req, res, next) => {
    ...
    try {
        ...
        if (studentFormFields.docs.length == 0) return res.status(404).json({ message: 'Field not found', status: false });
...
exports.deleteStudentFormFiled = async (req, res, next) => {
    ...
    if (role === 'admin' || role === 'super_admin') {
        ...
        try {
            ...
            if (!studentFormField) return res.status(404).json({ message: 'Field Not found', status: false });
...
exports.updateStudentFormFiled = async (req, res, next) => {
    ...
    if (role === 'admin') {
        ...
        try {
            const studentFormField = await StudentFormField.findByIdAndUpdate(id, { $set: req.body }, { validateBeforeSave: false })

            if (!studentFormField) return res.status(400).json({ message: 'Wrong input! try again..', status: false });
...
// get all fieltered StudentFormField 
exports.getAllFielteredStudentFormField = async (req, res, next) => {
    const { id, role } = res.locals.user;

    try {
        const studentFormFields = await StudentFormField.find({}).select('input_name status')

        if (studentFormFields.length == 0) return res.status(404).json({ message: 'Field not found', status: false });
